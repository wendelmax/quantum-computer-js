  ...
  const saveCurrentCircuit = () => {
    try {
      ...
      const newCircuit: CircuitGallery = {
        ...
        description: `${circuit.portas.length} gates on ${circuit.numQubits} qubits`,
        circuit,
    ...
    <div className="p-6 grid grid-cols-12 gap-4">
      <div className="col-span-9 flex flex-col gap-4">
        <div className="flex items-center justify-between">
          ...
          <Button onClick={saveCurrentCircuit}>Save Current Circuit</Button>
        </div>

        <Card>
          <p className="text-sm text-slate-300 mb-4">
            Save your favorite circuits and load them anytime. Circuitos são salvos localmente no navegador.
          </p>
        </Card>
            ...
            {savedCircuits.map(circuit => (
              <Card key={circuit.id} title={circuit.name} description={circuit.description}>
                ...
                <div className="flex gap-2">
                  <Button onClick={() => loadCircuit(circuit.id)}>Load</Button>
                  <Button variant="secondary" onClick={() => setSelectedCircuit(circuit.id)}>View</Button>
                  <Button variant="secondary" onClick={() => deleteCircuit(circuit.id)}>Delete</Button>
                </div>
              </Card>
            ))}
          </div>
