...
export default function CircuitCanvas({ children, selectedGate, onPlace, circuit, onRemove, onMove }: Props) {
  ...
  return (
    <div className="rounded-lg bg-[#021428] border border-slate-800 p-4">
      ...
      <div className={`rounded overflow-${hasScroll ? 'auto' : 'visible'}`} style={{ maxHeight: hasScroll ? `${maxDisplayHeight}px` : 'none', minHeight: `${Math.min(minHeight, maxDisplayHeight)}px` }}>
        <div className="p-3">
                ...
                {Array.from({length:maxCols},(_,col)=> (
                  ...
                  >
                    ...
                    {(() => {
                      ...
                      return (
                        <button
                          className="relative z-10 px-2 py-1 rounded bg-slate-800 border border-slate-700 text-sky-300 text-xs flex flex-col items-center min-w-[36px]"
                          draggable
