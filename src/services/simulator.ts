...
import SimWorker from '../workers/simWorker.ts?worker&inline'

let worker: Worker | null = null
function getWorker(): Worker | null {
  if (typeof window === 'undefined' || typeof Worker === 'undefined') return null
  try {
    if (!worker) worker = new SimWorker()
    return worker
  } catch {
    return null
  }
...
function applySingleQubit(state: Complex[], target: number, U: Complex[][]): Complex[] {
